...
- **Producción:** `oceanwatch_g07.analytics.ais_positions_d07_delta` es una tabla Delta administrada con `CLUSTER BY (MMSI, BaseDateTime)`, comentario de procedencia y limpieza D-07.
- **Línea base:** Parquet se conserva como archivos en el Volume gobernado `oceanwatch_g07.analytics.benchmark_files` y no es productivo. Databricks Free Edition no permite tablas administradas `USING PARQUET`; solo Delta. Esta es una restricción de plataforma, no una omisión del experimento.
